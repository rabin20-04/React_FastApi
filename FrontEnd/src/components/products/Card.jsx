...
const ProductsCard = ({ id, title,name, price, image_url }) => {
  ...
  return (
    <>
      <Link to={`/products/${id}`} className="border  shadow my-4 py-6 px-6  rounded-xl  relative bg-white hover:shadow-lg transition-transform duration-300 ease-in-out hover:scale-105 ">
        {/* ProductsCard */}
        <span className="text-xs text-white bg-red-600 rounded-xl my-2 px-1 py-1 absolute top-0 right-4 ">
          Clothing
        </span>
        <img src={white4} alt="" className=" w-full h-auto bg-white object-cover rounded-md " />
